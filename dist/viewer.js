...
function terrainBinFromMeta(recording) {
...
export function scanFrameEvents(room, frame, state, events) {
    let controller;
    let invaders = false;
    const users = new Set();
...
export function finishEventScan(room, state, events) {
 ...
 */
function ensureEvents(recording) {
...
// --- the API ----------------------------------------------------------------
function windowsOf(recording, room) {
    const bases = new Set();
...
function activityAt(recording, base) {
    const rows = [];
    for (const room of recording.rooms()) {
        let len = 0;
        for (const record of recording.index(room)) {
...
function sketch(object) {
    ...
    if (actionLog && typeof actionLog === 'object') {
        const kept = {};
        let any = false;
        for (const [key, value] of Object.entries(actionLog)) {
            if (value) {
                kept[key] = value;
                any = true;
            }
...
function chunkFor(recording, room, base) {
 ...
 */
export async function serveViewer(context, recording, rest, options) {
    const { meta } = recording;
    const totals = () => {
        let chunks = 0;
        let bytes = 0;
