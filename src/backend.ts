...
hooks.register('middleware', koa => {
	koa.use(async (context: RequestContext, next: () => Promise<unknown>) => {
		let recording: Recording | undefined = recorder?.recording;
		let prefix = '';

		// The frozen servers: /replay/<name>/...
		...
		if (context.path === '/replay' || context.path === '/replay/') {
			...
			if (name === 'flush' || name === 'status') {
				...
				context.body = { ok: 1, recording: recorder.recording.dir, saved, ...recorder.stats };
				return;
			}
			recording = openRecording(name);
			if (recording === undefined) {
				context.status = 404;
			...
			prefix = `/replay/${encodeURIComponent(name)}`;
			...
			} else if (rest === '/' || rest === '/index.html') {
				context.path = '/';
			} else if (rest === '/config.js') {
				context.type = 'text/javascript';
				context.set('Cache-Control', 'no-cache');
				context.body = clientConfig(prefix);
				return;
		...
		if (context.path === '/config.js' && prefix === '') {
			context.type = 'text/javascript';
			context.set('Cache-Control', 'no-cache');
