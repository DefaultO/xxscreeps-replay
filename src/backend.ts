...
/** A request under /replay/<name>/, as the front middleware saw it. */
interface Mount {
	name: string;
	recording: Recording;
	/** The path after the prefix, before any rewrite: `/api/game/time`, `/map/api/span`, ... */
	rest: string;
}

// What this middleware touches of the Koa context (the workspace has no koa
// typings of its own).
interface RequestContext {
	...
	state: { userId?: string; replay?: Mount };
	type: string;
	status: number;
	body: unknown;
	db: Database;
	shard: Shard;
	backend: { world: World; accessibleRooms: Set<string> };
	set(field: string, value: string): void;
	redirect(url: string): void;
}

type Middleware = (context: RequestContext, next: () => Promise<unknown>) => Promise<unknown>;

/** The replay code the client's history view carries: `<recording>~<room>`. */
function parseCode(code: string): { name: string; room: string } | undefined {
...
function clientConfig(prefix: string) {
	return `
...
function historyRoute(recording: Recording, room: string, base: number) {
	const size = config.historyChunkSize;
	const live = recorder?.recording === recording
		? (chunkIndex: number) => recorder!.liveFrames(room, chunkIndex)
		: undefined;
...
const front: Middleware = async (context, next) => {
	...
	if (context.path === '/build.min.js' && typeof context.body === 'string') {
		context.body = context.body.replace(
			/historyMinGameTime=Math\.floor\(\((\w+)\.time-15e4\)\/([\w.]+)\)\*([\w.]+),/,
			'historyMinGameTime=Math.max(0,Math.floor(($1.time-15e4)/$2)*$3),');
	}
};

// --- the main middleware: everything that needs the server -------------------

const main: Middleware = async (context, next) => {
	const mount = context.state.replay;
	let recording: Recording | undefined = mount?.recording ?? recorder?.recording;
	...
	if (control) {
		...
		if (name === 'badge' && context.method === 'POST') {
			...
			const badge = (context.request.body as { badge?: unknown } | undefined)?.badge;
			if (!userId) {
				context.status = 401;
	...
	if (context.path === '/api/version' && typeof context.body === 'object' && context.body !== null) {
		const body = context.body as { serverData?: { historyChunkSize?: number } };
		if (body.serverData) {
...
hooks.register('middleware', koa => {
	...
	(koa as unknown as { middleware: Middleware[] }).middleware.unshift(front);
	koa.use(main);
});
