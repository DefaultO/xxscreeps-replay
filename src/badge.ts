...
const files = new Map<string, BadgeFile | null>();

function readFile(file: string): BadgeFile | null {
	let stat: fs.Stats;
	try {
	...
	const entry: BadgeFile = { mtime: stat.mtimeMs };
	try {
		if (file.endsWith('.json')) {
			const parsed = JSON.parse(fs.readFileSync(file, 'utf8')) as unknown;
			// Either the badge itself or the whole `user/find` response
			const badge = (parsed as { badge?: unknown })?.badge ?? (parsed as { user?: { badge?: unknown } })?.user?.badge ?? parsed;
...
const safeName = (username: string) => /^[A-Za-z0-9_-]{1,32}$/.test(username) ? username : undefined;

/** The badge object from `<dir>/<username>.json`, if there is one. */
export function badgeFromFile(dir: string | undefined, username: string): unknown {
...
export function badgeSvgFromFile(dir: string | undefined, username: string): string | undefined {
	const name = dir && safeName(username);
