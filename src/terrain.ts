...
export function terrainDim(byteLength: number): number {
...
function gridFor(rooms: Iterable<{ wx: number; wy: number }>): { dim: number; tl: number } {
	let reach = 1;
...
function roomOffset(pos: { wx: number; wy: number }, dim: number, tl: number) {
	return ((pos.wy - tl) * dim + (pos.wx - tl)) * BYTES_PER_ROOM;
}

function encodeRoom(buffer: Buffer, offset: number, tile: (index: number) => number) {
...
export function terrainBinFromStrings(rooms: Iterable<{ name: string; terrain: string }>): Buffer | undefined {
 ...
 */
export function writeTerrainBin(file: string, world: { map: { getRoomTerrain(name: string): { get(x: number, y: number): number } } }, roomNames: Iterable<string>) {
...
export function roomFromTerrainBin(buffer: Uint8Array, room: string): string | undefined {
